...
def updateframe_callback(image_msgs):
    global frame
    frame=ros_numpy.numpify(image_msgs)
...
def main():
    ...
    while not rospy.is_shutdown():
       ...
       #     break
        ...
        res = detect_objects(interpreter, img, MIN_SCORE_THRESH)
        print(res)

        for index, result in enumerate(res):
            if index == MAX_BOX_TO_DRAW:
                break
            ...
            if index == 0:
                msg_label.label_1.data = labels[int(result['class_id'])]
            if index == 1:
                msg_label.label_2.data = labels[int(result['class_id'])]


        msg_img = ros_numpy.msgify(Image, frame, "bgr8")
